...
Safari.prototype.fetchStylesheet = function fetchStylesheet(id) {
  ...
  if (cssSet.size > 1) {
    logger.log(
      `an error occurred while fetching stylesheets. there are ${
        cssSet.size
      } different stylesheets. only supporting one. the layout may be different from the one online. sorry for that.`
    )
    debug(
      `an error occurred while fetching stylesheets. there are ${
        cssSet.size
      } different stylesheets. only supporting one.`
    )
...
Safari.prototype.authorizeUser = function authorizeUser(username, password) {
  ...
  return request(options)
    ...
    .catch(err => {
      // ### an error occured
      debug(
        `an error occured while trying to fetch authorization token (error: ${
          err
        })`
      )
...
Safari.prototype.fetchResource = function fetchResource(url, options) {
  ...
  if (!url || (!this.accessToken && !this.sessionId))
    return Promise.reject(
...
Safari.prototype.fetchBookById = function fetchBookById(
  ...
  if (sessionId) {
    ...
    task = this.fetchMeta(id)
  } else {
    task = this.authorizeUser(username, password).then(accessToken => {
